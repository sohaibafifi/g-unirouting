...
def _constraint_item_label(item: dict) -> str:
    state_name = str(item.get("constraint_state", item.get("constraint", ""))).strip()
    family_name = str(item.get("constraint_family", "")).strip()
    state_label = CONSTRAINT_LABELS.get(state_name, state_name)
    if family_name and family_name != state_name:
        family_label = CONSTRAINT_LABELS.get(family_name, family_name)
...
def _plot_constraint_share_compare(
    ax,
    input_share: object,
    decoder_state_share: object,
    title: str,
) -> None:
    input_items = _top_share_items(input_share, limit=8)
    decoder_items = _top_share_items(decoder_state_share, limit=8)
    names = list(
        dict.fromkeys([name for name, _ in input_items] + [name for name, _ in decoder_items])
    )
    if not names:
        ax.text(0.5, 0.5, "No constraint summary", ha="center", va="center")
        ax.set_axis_off()
        return

    input_map = {name: value for name, value in input_items}
    decoder_map = {name: value for name, value in decoder_items}
    labels = [CONSTRAINT_LABELS.get(name, name) for name in names]
    base_colors = [CONSTRAINT_COLORS.get(name, CONSTRAINT_COLORS["other"]) for name in names]
    pos = np.arange(len(names), dtype=float)
    height = 0.36

    ax.barh(
        pos - height / 2,
        [input_map.get(name, 0.0) for name in names],
        height=height,
        color=base_colors,
        alpha=0.45,
        label="input attribution",
    )
    ax.barh(
        pos + height / 2,
        [decoder_map.get(name, 0.0) for name in names],
        height=height,
        color=base_colors,
        alpha=0.9,
        hatch="//",
        label="decoder dynamic families",
    )
    max_value = max(
        [input_map.get(name, 0.0) for name in names]
        + [decoder_map.get(name, 0.0) for name in names]
...
def _dominant_constraint_over_steps(step_payloads: object) -> tuple[str, float]:
    shares = _aggregate_step_payload_shares(step_payloads, "constraint")
    if not shares:
        return "other", 0.0
    name, share = max(shares.items(), key=lambda item: (item[1], item[0]))
    return str(name), float(share)
...
def _trajectory_plot_summary(
    ...
    decoder_state_constraints: List[object],
    decoder_dynamic_states: List[object],
    ...
    decoder_state_constraints = decoder_state_constraints[:max_step]
    decoder_dynamic_states = decoder_dynamic_states[:max_step]
    ...
    regime_source = (
        "decoder dynamic states"
        if (decoder_dynamic_states or decoder_state_constraints)
        else "input attribution"
    )
    split_idx = max(1, len(regime_payloads) // 2)
    early = _dominant_constraint_over_steps(regime_payloads[:split_idx])[0]
    late = _dominant_constraint_over_steps(
        regime_payloads[split_idx:] or regime_payloads[:split_idx]
    )[0]
    ...
    if early_label and late_label and early_label != late_label:
        lines.append(f"Régime ({regime_source}): {early_label} -> {late_label}")
    elif early_label:
        lines.append(f"Régime ({regime_source}): stable ({early_label})")
    return lines
...
def _plot_global_trajectory_summary(
    ...
    for payload in regime_payloads:
    ...
    ax_constraints.set_title(
        f"Dominant Dynamic Signal Per Step ({regime_title_suffix})"
    )
...
def _plot_method_summary(
    ...
    _plot_constraint_share_compare(
        ax_constraints,
        summary.get("constraint_importance_share", {}),
        summary.get("decoder_state_constraint_importance_share", {}),
        "Constraint Families",
    )
    ...
    early_name = str(trajectory.get("early_top_decoder_dynamic_state", "")).strip()
    late_name = str(trajectory.get("late_top_decoder_dynamic_state", "")).strip()
    regime_source = "decoder dynamic states"
    if not early_name and not late_name:
        early_name = str(
            trajectory.get("early_top_decoder_state_constraint_state", "")
        ).strip()
        late_name = str(
            trajectory.get("late_top_decoder_state_constraint_state", "")
        ).strip()
        regime_source = "decoder dynamic states"
    if not early_name and not late_name:
        early_name = str(trajectory.get("early_top_decoder_state_constraint", "")).strip()
        late_name = str(trajectory.get("late_top_decoder_state_constraint", "")).strip()
        regime_source = "decoder dynamic families"
    if not early_name and not late_name:
        early_name = str(trajectory.get("early_top_constraint", "")).strip()
        late_name = str(trajectory.get("late_top_constraint", "")).strip()
        regime_source = "input attribution"
    summary_lines = [
        f"model: {model_label}",
    ...
    if early_name or late_name:
        ...
        if early_label == late_label and early_label != "n/a":
            summary_lines.append(f"regime ({regime_source}): stable ({early_label})")
        else:
            summary_lines.append(f"regime ({regime_source}): {early_label} -> {late_label}")
    ax_text.text(
        0.02,
        0.98,
        "\n".join(summary_lines),
...
def _plot_instance_summary(
    ...
    decoder_state_constraints_all = trace.get("decoder_state_constraints", []) or []
    ...
    regime_payloads = (
        decoder_dynamic_states_all
        or decoder_state_constraints_all
        or top_constraints_all
    )
    regime_source = (
        "decoder dynamic states"
        if (decoder_dynamic_states_all or decoder_state_constraints_all)
        else "input attribution"
    )
    constraint_share = _aggregate_step_payload_shares(
        top_constraints_all,
        "constraint",
    )
    decoder_state_constraint_share = _aggregate_step_payload_shares(
        decoder_dynamic_states_all or decoder_state_constraints_all,
        "constraint",
    )
    ...
    split_idx = max(1, len(regime_payloads) // 2)
    early_name = _dominant_constraint_over_steps(regime_payloads[:split_idx])[0]
    late_name = _dominant_constraint_over_steps(
        regime_payloads[split_idx:] or regime_payloads[:split_idx]
    )[0]
    ...
    _plot_constraint_share_compare(
        ax_constraints,
        constraint_share,
        decoder_state_constraint_share,
        "Constraint Families",
    )
    ...
    late_label = CONSTRAINT_LABELS.get(late_name, late_name or "n/a")
    if early_label == late_label and early_label != "n/a":
        text_lines.append(f"regime ({regime_source}): stable ({early_label})")
    else:
        text_lines.append(f"regime ({regime_source}): {early_label} -> {late_label}")
    ax_text.text(
        0.02,
        0.98,
        "\n".join(text_lines),
...
def _plot_single_report(
    ...
    for idx in instance_ids:
        ...
        _plot_global_trajectory_summary(
            ...
            actions,
            top_constraints_all,
            trace.get("decoder_state_constraints", []) or [],
            _trace_decoder_dynamic_states(trace),
            recourse_flags,
            chosen_feasible_all,
            done_step,
        )
        ...
        for step in selected_steps:
            ...
            decoder_state_constraints_step = (
                (trace.get("decoder_state_constraints", []) or [])[step]
                if step < len(trace.get("decoder_state_constraints", []) or [])
                else []
            )
            decoder_dynamic_states_all = _trace_decoder_dynamic_states(trace)
            decoder_dynamic_states_step = (
            ...
            _plot_step_payload_bars(
                axes[1, 0],
                decoder_dynamic_states_step
                or decoder_state_constraints_step
                or top_constraints_step,
                field_name="constraint",
                title=(
                    "Top Decoder Dynamic States (Step)"
                    if (decoder_dynamic_states_step or decoder_state_constraints_step)
                    else "Top Input Constraint Families (Step)"
                ),
            )
            _plot_step_payload_bars(
                axes[1, 1],
