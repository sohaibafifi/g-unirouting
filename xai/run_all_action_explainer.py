...
def _report_matches_spec(
    ...
    try:
        ...
        wanted_topk = tuple(
            sorted({int(v.strip()) for v in str(args.topk_nodes).strip("[]").split(",") if v.strip()})
        )
    except Exception:
        return False
    ...
    try:
        report_top_m = int(report_cfg.get("feasibility_top_m", -1))
    except Exception:
        return False
    if report_top_m != int(args.feasibility_top_m):
        return False

    try:
        report_cost_weight = float(report_cfg.get("feasibility_cost_weight", -1.0))
    except Exception:
        return False
    if report_cost_weight != float(args.feasibility_cost_weight):
        return False
