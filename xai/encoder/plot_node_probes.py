...
from encoder.node_concept_bank import NODE_CONCEPT_CLASS_ORDERS, NODE_CONCEPT_DISPLAY_NAMES
...
def _load_probe_artifacts(report: Dict[str, Any]) -> Dict[str, Any]:
    artifact_path = _nested_get(report, ["artifacts", "customer_features_path"])
    if not artifact_path:
        raise FileNotFoundError(
            "Missing customer feature artifact path in report. Re-run compare_node_probes.py "
            "with the updated version to generate .npz sidecars."
        )
    path = Path(str(artifact_path))
    if not path.exists():
        raise FileNotFoundError(f"Node probe artifact not found: {path}")

    loaded = np.load(path, allow_pickle=False)
    artifacts: Dict[str, Any] = {
        "customer_features": loaded["customer_features"],
        "probe_core_node_signatures": loaded["probe_core_node_signatures"].astype(str),
        "node_concept_states": {},
    }
    for key in loaded.files:
        if key.startswith("node_concept_state__"):
            artifacts["node_concept_states"][key.split("__", 1)[1]] = loaded[key].astype(str)
    return artifacts
...
def _node_concept_order() -> List[str]:
    return [
        "service_order_state",
        "route_role_state",
...
def _node_concept_display_label(concept_name: str) -> str:
    return NODE_CONCEPT_DISPLAY_NAMES.get(concept_name, concept_name)


def _class_order(concept_name: str, classes: Sequence[str]) -> List[str]:
    preferred = NODE_CONCEPT_CLASS_ORDERS.get(concept_name, [])
...
def _plot_node_heatmap(
    rows: Sequence[Dict[str, Any]],
    reports_by_model: Dict[str, Dict[str, Any]],
    output_dir: Path,
    dpi: int,
) -> Path:
    concept_names = _node_concept_order()
    ...
    for row_idx, row in enumerate(rows):
        ...
        for col_idx, concept_name in enumerate(concept_names):
            matrix[row_idx, col_idx] = _num_or_nan(
                _nested_get(report, ["node_concept_state_separation", concept_name, "linear_probe", "macro_f1"])
            )
    ...
    ax.set_title("Node Concept Macro-F1")
    ax.set_yticks(np.arange(len(labels)))
    ax.set_yticklabels(labels)
    ax.set_xticks(np.arange(len(concept_names)))
    ax.set_xticklabels(
        [_node_concept_display_label(name) for name in concept_names],
        rotation=20,
        ha="right",
    )
    ...
    path = output_dir / "node_concept_macro_f1_heatmap.png"
...
def _plot_signature_bars(rows: Sequence[Dict[str, Any]], output_dir: Path, dpi: int) -> Path:
    ...
    metrics: List[Tuple[str, str, str]] = [
        ("node_macro_f1_mean", "Node Mean Macro-F1", "#72b7b2"),
        ("node_signature_nmi", "Node Signature NMI", "#4c78a8"),
        ("node_signature_ari", "Node Signature ARI", "#f58518"),
    ]
    ...
    path = output_dir / "node_signature_bars.png"
    fig.savefig(path, dpi=dpi, bbox_inches="tight")
    plt.close(fig)
    return path
...
def _plot_rank_vs_signature(
    ...
    ys = [_num_or_nan(row.get("node_signature_nmi")) for row in rows]
    sizes = []
    for row in rows:
        node_mean = _safe_float(row.get("node_macro_f1_mean"))
        sizes.append(160 if node_mean is None else 120 + 180 * node_mean)
    ...
    ax.set_ylabel("Node concept signature NMI")
    ax.set_title("Latent Richness vs Node-Concept Organization")
    ax.grid(alpha=0.25)

    path = output_dir / "rank_vs_node_signature_nmi.png"
    fig.savefig(path, dpi=dpi, bbox_inches="tight")
    plt.close(fig)
    return path
...
def _plot_projection_grid(
    ...
    for row_idx, row in enumerate(rows):
        ...
        features = artifacts["customer_features"]
        for col_idx, concept_name in enumerate(concept_names):
            ax = axes[row_idx, col_idx]
            labels = artifacts["node_concept_states"][concept_name]
            ...
            if row_idx == 0:
                ax.set_title(_node_concept_display_label(concept_name))
            if col_idx == 0:
    ...
    path = output_dir / f"node_concept_projections_{method}.png"
    fig.savefig(path, dpi=dpi, bbox_inches="tight")
    plt.close(fig)
    return path
...
def main() -> None:
    ...
    generated: List[Path] = []
    generated.append(_plot_node_heatmap(rows, reports_by_model, output_dir, args.dpi))
