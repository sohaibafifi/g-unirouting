...
from encoder.edge_concept_bank import compute_edge_concept_bank
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Quantitative edge-level probe of encoder latent spaces using solution-derived edge concepts."
    )
...
def _pair_features(
    ...
    src = node_matrix[instance_indices, src_indices, :]
    dst = node_matrix[instance_indices, dst_indices, :]
...
def compute_edge_probe_bundle(args: argparse.Namespace) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    edge_concept_bank = compute_edge_concept_bank(
    ...
    concept_states = edge_concept_bank["concept_states"]
    concept_raw_values = edge_concept_bank["concept_raw_values"]
    core_signatures = edge_concept_bank["core_concept_signatures"]
    probe_core_signatures = _collapse_rare_labels(core_signatures, min_count=4)

    edge_feature_matrix = _pair_features(
        node_matrix=node_matrix,
        instance_indices=edge_concept_bank["instance_indices"],
        src_indices=edge_concept_bank["src_indices"],
        dst_indices=edge_concept_bank["dst_indices"],
    )

    clustering_k_values = list(
        range(
            2,
            min(max(args.max_k, 2), edge_feature_matrix.shape[0] - 1) + 1,
        )
    )
    clustering_sweep = [
        _kmeans_metrics(edge_feature_matrix, k=k_value, seed=args.seed)
    ...
    for concept_name, labels in sorted(concept_states.items()):
        ...
        concept_probes[concept_name] = {
            "silhouette": _label_silhouette(edge_feature_matrix, labels, seed=args.seed),
            "centroid_margin": _centroid_margin(edge_feature_matrix, labels),
            "linear_probe": linear_probe,
            "class_balance": dict(Counter(labels)),
            "display_name": edge_concept_bank["concept_display_names"].get(concept_name, concept_name),
        }
    ...
    report = {
        "config": {
            ...
            "max_probe_edges": int(args.max_probe_edges),
        ...
        "dataset": {
            ...
            "num_probe_edges": int(edge_feature_matrix.shape[0]),
            "num_core_edge_signatures": int(len(set(core_signatures))),
            "core_edge_signature_counts": dict(Counter(core_signatures)),
            "num_probe_core_edge_signatures": int(len(set(probe_core_signatures))),
            "probe_core_edge_signature_counts": dict(Counter(probe_core_signatures)),
        },
        "matrix_richness": _matrix_richness(node_matrix),
        "edge_concept_bank": {
            "core_concepts": list(edge_concept_bank["core_concept_names"]),
            "display_names": dict(edge_concept_bank["concept_display_names"]),
            "class_orders": dict(edge_concept_bank["concept_class_orders"]),
            "raw_value_summary": dict(edge_concept_bank["raw_value_summary"]),
        ...
        "edge_concept_signature_separation": {
            "label_silhouette": _label_silhouette(
                edge_feature_matrix,
                probe_core_signatures,
                seed=args.seed,
            ),
            "centroid_margin": _centroid_margin(edge_feature_matrix, probe_core_signatures),
            "linear_probe": _cross_validated_probe(
                edge_feature_matrix,
                probe_core_signatures,
                seed=args.seed,
        ...
        "edge_concept_state_separation": concept_probes,
        "clustering_sweep": clustering_sweep,
        "best_k_by_silhouette": best_silhouette,
    }
    artifacts = {
        "edge_features": edge_feature_matrix.astype(np.float32, copy=False),
        "core_edge_signatures": np.asarray(core_signatures),
        "probe_core_edge_signatures": np.asarray(probe_core_signatures),
        "edge_concept_states": {
            name: np.asarray(values)
            for name, values in sorted(concept_states.items())
        },
        "edge_concept_raw_values": {
        ...
        "src_indices": edge_concept_bank["src_indices"],
        "dst_indices": edge_concept_bank["dst_indices"],
        "relation_labels": np.asarray(edge_concept_bank["relation_labels"]),
    }
    return report, artifacts


def write_edge_probe_report(report: Dict[str, Any], output_path: str) -> Path:
...
def write_edge_probe_artifacts(artifacts: Dict[str, Any], output_path: str) -> Path:
    path = Path(output_path)
    path.parent.mkdir(parents=True, exist_ok=True)
    flattened: Dict[str, Any] = {
        "edge_features": artifacts["edge_features"],
        "core_edge_signatures": artifacts["core_edge_signatures"],
        "probe_core_edge_signatures": artifacts["probe_core_edge_signatures"],
        "instance_indices": artifacts["instance_indices"],
        "src_indices": artifacts["src_indices"],
        "dst_indices": artifacts["dst_indices"],
        "relation_labels": artifacts["relation_labels"],
    }
    for name, values in sorted((artifacts.get("edge_concept_states") or {}).items()):
        flattened[f"edge_concept_state__{name}"] = values
    for name, values in sorted((artifacts.get("edge_concept_raw_values") or {}).items()):
        flattened[f"edge_concept_value__{name}"] = values
...
def main() -> None:
    ...
    report, artifacts = compute_edge_probe_bundle(args)

    if args.artifacts_output:
        artifact_path = write_edge_probe_artifacts(artifacts, args.artifacts_output)
        report["artifacts"] = {
            "edge_features_path": str(artifact_path),
        }

    output_path = write_edge_probe_report(report, args.output)

    print(
        f"Edge probe saved to {output_path}\n"
        f"encoder={report['config']['encoder_name']} decode={report['config']['decode_mode']} "
        f"instances={report['dataset']['num_instances']} "
        f"edges={report['dataset']['num_probe_edges']}"
    )
